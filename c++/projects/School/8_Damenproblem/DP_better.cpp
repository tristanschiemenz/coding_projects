    ...
    for(int row = 0; row < Field.size(); row++){
        ...
        for(int col = 0; col < Field[row].size(); col++){
            ...
            if(Field[row][col] != 1){
                ...
                if(check_field_state(Field) && placed != N){
                    placed++;
                    NDamen(N,Field);
                    placed--;
                }//Wenn sie gesetz werden kann und es N Damen auf dem Feld sind(Lösung)
                else if(check_field_state(Field) && placed == N){
                    print_array(Field);
...
void better_NDamen(int N, vector<vector<int>> Field,int INlevel_station){
    QueensPlaced++;
    for(int col = 0; col < Field.size(); col++){

        if(Field[INlevel_station][col] == 0){

            Field[INlevel_station][col] = 1;
            bool fieldState = check_field_state(Field);

            if((fieldState)&& (INlevel_station != N-1)){
                better_NDamen(N,Field,INlevel_station+1);

            }else if((fieldState) && (INlevel_station == N-1)){
                print_array(Field);
            }
            Field[INlevel_station][col] = 0;
        }
    }
}
//0 - keine Dame
...
int main(){
    ...
    if (HowManyQueens <= HowBigField){
        ...
        better_NDamen(HowManyQueens,Field,0);
        ...
        cout << "Es wurde " + to_string(FieldChecks) + " mal das Feld überprüft und es wurden " + to_string(QueensPlaced) +" Damen  ausprobiert";
